...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    boolean isService = false; // 서비스가 동작중인지를 체크하는 변수

    ServiceConnection conn = new ServiceConnection(){
        // 서비스와 연결되었을 때 호출되는 함수
        @Override
        public void onServiceConnected(ComponentName name, IBinder binder) {    // 서비스의 onBind에서 리턴되는 값이 binder에 담겨온다.
            MyService.MyBinder mb = (MyService.MyBinder) binder;
            bService = mb.getService();
            isService = true;
        }
        @Override
        public void onServiceDisconnected(ComponentName name) {     // 서비스가 중단되거나 연결이 도중에 끊겼을 때 발생함, onDestroy에서는 호출되지 않는다.
            isService = false;
        }
    };
